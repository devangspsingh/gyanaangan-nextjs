...
// Request interceptor to add the token and visitor ID
api.interceptors.request.use(

  async (config) => {
    // Add Auth Token (handled by headers manipulation mostly, but good to be safe if strictly needed here)
    // Note: The original code relied on api.defaults or manual usage. 
    // We will append Visitor ID here.
    try {
      // Use await to ensure we get the ID. The backend now allows 'X-Visitor-Id' header, preventing CORS errors.
      const visitorId = await AnalyticsService.getVisitorId();
      if (visitorId) {
        config.headers['X-Visitor-Id'] = visitorId;
      }
    } catch (e) {
      // Ignore analytics errors to not block app
    }
    return config;
  },
  (error) => {
